...
# ==============================================================================
#   機能
#     ディレクトリリストに記載されたディレクトリのオーナ・グループ・パーミッションを設定する
#   構文
#     USAGE 参照
#
...
while read line ; do
	...
	if [ $? -ne 0 ];then
		...
		if [ "${dir}" = "" -o "${uname}" = "" -o "${gname}" = "" -o "${mod}" = "" ];then
		...
		# 既存ディレクトリでない場合
			...
			echo "-W \"${dir}\" directory not exist, aborted" 1>&2
			POST_PROCESS;exit 1
		fi
	fi
done < "${DIR_LIST}"
